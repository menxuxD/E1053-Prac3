...
class myDB {
  ...
  getMetaData(name,callback){
	var fname = this.filename(name); //nombre del fichero
	
        //abrir y quedarnos con la primera linea -> parseamos el string y obtenemos el objeto JSON con el atributo track
	var fline = firstline(fname);
        fline.then(line => {
          var json = JSON.parse(line);
          //devolvemos name y track que lo necesitamos para construir el grafo
	  callback(json.track); //json.track json["track"]
